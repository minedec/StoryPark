...
export default function StoryPage1() { // 设置状态来追踪显示哪个背景图像
  ...
  const toggleRecording = () => {
    ...
    if (!recording) {
      ...
      mediaRecorder.onstop = () => { // 1. 定义一个函数用于将文本转换为音频并发送到指定端口
    const convertTextToVoice = async (text) => {
      try {
    // 发送POST请求到指定端口
      const response = await fetch('/text2voice', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ text: text }) // 将文本作为JSON数据发送
    });

    // 如果请求成功，返回响应
     if (response.ok) {
      return await response.blob(); // 将响应体作为二进制数据返回
    } else {
